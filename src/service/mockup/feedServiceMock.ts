...
@Resolver((_of) => Feed)
export class FeedResolverServiceMock implements FeedService {
  private feedMock: Feed[] = [
    {
      id: 1,
  ...
  feeds(paginated?: PaginationArg): Promise<Feed[]> {
    if (!paginated) {
      ...
      return Promise.resolve(
        this.feedMock.slice(paginated.skip, paginated.skip + paginated.take)
      );
    }
  }
    ...
    if (!paginated) {
      ...
      return Promise.resolve(
        this.articlesMock
          ...
          .slice(paginated?.skip, paginated?.skip + paginated?.take)
      );
    }
  }
